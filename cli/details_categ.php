<?php
    ...
    if (isset($_GET['id'])) {
        ...
        $entree_rech = $categ["lib_categ"];
        $type_rech = 'catégorie';
    
        $stmt_check->bind_param("ssiii", $entree_rech, $type_rech, $id_categ, $id_pers, $id_role);
        $stmt_check->execute();
        ...
        if ($count == 0) {
            ...
            $stmt1->bind_param("ssiii", $entree_rech, $type_rech, $id_categ, $id_pers, $id_role);
            $stmt1->execute();
            $stmt1->close();
        }

        $sql2 = "SELECT * FROM produits WHERE id_categ =?
        ORDER BY id_prod DESC;";
        $stmt2 = $conn->prepare($sql2);
        $stmt2->bind_param("i",$id_categ);
        $stmt2->execute();

        $result2 = $stmt2->get_result();
        $lesproduits = $result2->fetch_all(MYSQLI_ASSOC);
        $nbr_prod = count($lesproduits);
...
<body>
    ...
    <div class="card">
        <h2><?php echo htmlspecialchars($categ["lib_categ"])?></h2>
        <p><?php echo htmlspecialchars($categ["desc_categ"])?></p>
        <div class="number"> <?php echo $nbr_prod?> produit(s)</div>
    </div>
    <div class="prod-card">
            ...
            <?php foreach($lesproduits as $produit):?>
                <div class="product-item">
                    ...
                    <div class="product-info">
                        ...
                        <a class="add-to-cart" href="">Commander</a>
                    </div>
                </div>
            <?php endforeach;?>
        </div>
