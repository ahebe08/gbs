<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        $sql1 = "UPDATE categories SET lib_categ = ?, desc_categ = ? WHERE id_categ = ?";
        $stmt1 = $conn->prepare($sql1);
        $stmt1->bind_param("ssi", $nom_categ, $descr_categ, $id_categ);

        if ($stmt1->execute()) {
            header("Location: success_modif_categ.html");
            exit();
        } else {
...
<head>
    ...
    <title>GBS | Modifier catégorie</title>
    ...
    <main class="content">
        ...
        <form action="" method="POST">
            <div class="form-group">
                ...
                <input type="text" id="nom_categ" name="nom_categ" class="form-input" value="<?php echo $lacategorie['lib_categ']?>" required>
            </div>
            <div class="form-group">
                <label for="descr_categ" class="form-label">Brève description de la catégorie</label>
                <textarea id="descr_categ" name="descr_categ" class="form-textarea" value="" required><?php echo $lacategorie['desc_categ']?></textarea>
            </div>
